...
export default function Sidebar({ role }: SidebarProps) {
  const { unreadCount, loading: notificationsLoading, error: notificationsError } = useNotifications();
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger className="w-full flex items-center justify-between px-3 py-2 rounded-md text-sm font-medium transition-all group text-slate-600 hover:bg-slate-50 hover:text-slate-900 outline-none cursor-pointer">
                      <span className="flex items-center gap-3">
                        ...
                        {link.name}
                        {!notificationsLoading && !notificationsError && unreadCount > 0 && <span aria-label={`${unreadCount} unread notifications`} className="rounded-full bg-blue-100 px-2 text-xs text-blue-700">{unreadCount}</span>}
                      </span>
