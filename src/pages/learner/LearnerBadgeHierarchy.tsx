              ...
              <CardContent className="pt-5">
                {competencyNodes.length > 0 && (
                  <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                    {competencyNodes.map(({ competency, status }) => (
                ...
                <div className={`rounded-lg border p-5 ${statusStyle[completionStatus]}`}>
                  <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                    <div><p className="text-xs font-semibold uppercase tracking-wider">Complete standard</p><p className="mt-1 text-lg font-bold">{completionType}</p></div>
                    <StatusBadge status={completionStatus} />
                  </div>
                </div>
              </CardContent>
